...
CREATE TABLE IF NOT EXISTS `infocimol`.`usuario` (
  ...
  INDEX `fk_usuario_pessoa1_idx` (`pessoa_id_pessoa` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `infocimol`.`professor` (
  `pessoa_id_pessoa` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`pessoa_id_pessoa`),
  INDEX `fk_professor_pessoa1_idx` (`pessoa_id_pessoa` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `infocimol`.`projeto` (
  ...
  `abstract` TEXT(300) NOT NULL,
  `objetivo_geral` VARCHAR(100) NOT NULL,
  `objetivo_especifico` VARCHAR(255) NOT NULL,
  `arquivo` JSON NOT NULL,
  `publico` TINYINT NOT NULL,
  PRIMARY KEY (`id_projeto`),
  UNIQUE INDEX `idprojeto_UNIQUE` (`id_projeto` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `infocimol`.`orientacao` (
  ...
  INDEX `fk_orientacao_projeto1_idx` (`projeto_id_projeto` ASC) VISIBLE,
  INDEX `fk_orientacao_professor1_idx` (`professor_pessoa_id_pessoa` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `infocimol`.`aluno` (
  ...
  INDEX `fk_aluno_pessoa1_idx` (`pessoa_id_pessoa` ASC) VISIBLE,
  PRIMARY KEY (`pessoa_id_pessoa`),
...
CREATE TABLE IF NOT EXISTS `infocimol`.`aluno_projeto` (
  ...
  INDEX `fk_aluno_projeto_projeto1_idx` (`projeto_id_projeto` ASC) VISIBLE,
  CONSTRAINT `fk_aluno_projeto_projeto1`
    FOREIGN KEY (`projeto_id_projeto`)
    REFERENCES `infocimol`.`projeto` (`id_projeto`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_aluno_projeto_aluno1`
...
CREATE TABLE IF NOT EXISTS `infocimol`.`coordenacao` (
  ...
  INDEX `fk_coordenacao_professor1_idx` (`professor_pessoa_id_pessoa` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `infocimol`.`aluno_curso` (
  ...
  PRIMARY KEY (`aluno_pessoa_id_pessoa`),
...
-- -----------------------------------------------------
-- Table `infocimol`.`codigo_usuario`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `infocimol`.`codigo_usuario` (
  `codigo` INT(10) NOT NULL,
  `codigo_prazo` TIMESTAMP(6) NOT NULL,
  `codigo_usado` TINYINT NOT NULL,
  `usuario_pessoa_id_pessoa` INT UNSIGNED NOT NULL,
  INDEX `fk_codigo_usuario_usuario1_idx` (`usuario_pessoa_id_pessoa` ASC) VISIBLE,
  CONSTRAINT `fk_codigo_usuario_usuario1`
    FOREIGN KEY (`usuario_pessoa_id_pessoa`)
    REFERENCES `infocimol`.`usuario` (`pessoa_id_pessoa`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
